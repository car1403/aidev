...
이번 프로젝트는 단순히 챗봇 화면을 만드는 과제가 아닙니다. 기술 지원 업무를 자동화하는 AI 워크플로우를 설계하고, 노코드·로코드 도구로 구현 가능한 구조를 만들고, 테스트 결과를 문서로 검증해야 합니다.
...
수업 진행에서는 먼저 “어떤 기술 지원 문제가 반복적으로 발생하는가?”를 질문하면 좋습니다. 예를 들어 비밀번호 초기화, 사내 시스템 접속 오류, 소프트웨어 설치 문의, 장애 신고, 권한 요청 같은 주제는 워크플로우 설계 연습에 적합합니다.

주제를 정한 뒤 바로 구현부터 시작하지 말고, Trigger, 입력 데이터, 처리 노드, 출력 결과, 예외 상황을 먼저 문서로 정리해야 합니다.
