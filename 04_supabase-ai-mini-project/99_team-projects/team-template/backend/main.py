...
from pathlib import Path  # 현재 파일 위치를 기준으로 04 과정 최상위 .env 경로를 계산하기 위해 가져옵니다.
...
COURSE_ROOT = Path(__file__).resolve().parents[3]  # backend/main.py에서 04_supabase-ai-mini-project 최상위 폴더로 올라갑니다.
...
SUPABASE_KEY = os.getenv("SUPABASE_SERVICE_ROLE_KEY") or os.getenv("SUPABASE_ANON_KEY")  # 백엔드는 service role key를 우선 사용하고, 없으면 anon key로 대체합니다.

app = FastAPI(title="Supabase Service Log API")  # FastAPI 서버 객체를 생성합니다. 이후 데코레이터로 API 경로를 이 객체에 등록합니다.


class ServiceLogCreate(BaseModel):  # Pydantic 모델을 정의합니다. FastAPI가 요청 본문을 이 구조로 검증합니다.
    action: str = Field(min_length=1, max_length=100)  # 어떤 작업이 실행되었는지 기록합니다.
    status: str = Field(default="success", pattern="^(success|fail|warning)$")  # 실행 결과를 success, fail, warning 중 하나로 제한합니다.
    metadata: dict = Field(default_factory=dict)  # 실행 시간, 모델명, 오류 코드 같은 추가 정보를 JSON 형태로 저장합니다.


class ServiceLogUpdate(BaseModel):  # Pydantic 모델을 정의합니다. FastAPI가 요청 본문을 이 구조로 검증합니다.
    status: str = Field(pattern="^(success|fail|warning)$")  # 로그 상태를 정해진 값 중 하나로만 수정할 수 있게 제한합니다.
...
def health_check():  # 서버와 외부 연결 상태를 확인하는 health check 요청을 처리합니다.
    ...
    response = supabase.table("service_logs").select("id").limit(1).execute()  # Supabase 테이블에서 데이터를 조회하고 응답 객체에 저장합니다.
...
def list_logs():  # 저장된 데이터를 조회하는 요청을 처리합니다.
    supabase = get_supabase_client()  # Supabase 테이블 조회와 저장에 사용할 클라이언트 객체를 가져옵니다.
    response = supabase.table("service_logs").select("*").order("created_at", desc=True).execute()  # Supabase 테이블에서 데이터를 조회하고 응답 객체에 저장합니다.
    return {"items": response.data}  # API 클라이언트가 받을 JSON 형태의 응답 데이터를 반환합니다.


@app.post("/api/logs")  # HTTP POST 요청을 처리할 API 엔드포인트를 등록합니다.
def create_log(payload: ServiceLogCreate):  # 요청 본문으로 받은 데이터를 새 항목으로 저장합니다.
    supabase = get_supabase_client()  # Supabase 테이블 조회와 저장에 사용할 클라이언트 객체를 가져옵니다.
    response = supabase.table("service_logs").insert(payload.model_dump()).execute()  # Supabase 테이블에 새 행을 추가하고 저장 결과를 응답 객체에 저장합니다.
...
def update_log(log_id: str, payload: ServiceLogUpdate):  # 경로 변수와 요청 본문을 사용해 기존 데이터를 수정합니다.
    supabase = get_supabase_client()  # Supabase 테이블 조회와 저장에 사용할 클라이언트 객체를 가져옵니다.
    response = supabase.table("service_logs").update(payload.model_dump()).eq("id", log_id).execute()  # Supabase 테이블의 기존 행을 수정하고 결과를 응답 객체에 저장합니다.
    if not response.data:  # Supabase 작업 결과가 비어 있으면 대상 데이터를 찾지 못한 상황으로 처리합니다.
        raise HTTPException(status_code=404, detail="Service log not found")  # API 요청을 정상 처리할 수 없을 때 HTTP 오류 응답을 반환합니다.
