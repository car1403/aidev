...
이 과정에서는 Docker로 PostgreSQL이나 Redis를 실행하지 않습니다. 데이터 저장은 Supabase 프로젝트를 사용하고, 로컬 PC에서는 FastAPI와 Streamlit만 Python 가상환경으로 실행합니다. Docker, Docker Compose, AWS 배포는 `07_multi-agent-service-ops`에서 학습합니다.
...
`04_supabase-ai-mini-project` 최상위 폴더에서 `.env.example`을 `.env`로 복사합니다.
...
`C:\aidev\.env`, `02_supabase-ai-backend\.env`, `03_supabase-ai-frontend\.env`를 사용하는 구조가 아닙니다.
...
이 과정에서는 `04_supabase-ai-mini-project` 최상위 `.venv` 하나를 사용합니다.
