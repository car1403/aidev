...
강사 샘플 또는 팀 템플릿을 실행할 때는 해당 폴더 안에서 `.env.example`을 `.env`로 복사합니다.
...
AWS는 비용이 발생할 수 있으므로 실제 배포는 강사 안내에 따라 진행합니다.
...
5. 강사 샘플 docker compose config 확인
6. 강사 샘플 docker compose up --build 실행
7. backend /health 확인
