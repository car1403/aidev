...
앞 단원인 `02_llm-api-integration`의 변경 방향에 맞춰 실제 LLM 연동은 Gemini SDK를 기본 확장 방향으로 봅니다. 이 단원에서는 실제 Gemini SDK를 바로 호출하지 않고, `provider`, `model`, `actual_api_called`, `llm_call_mode` 값을 저장하면서 나중에 실제 호출 함수로 교체할 준비를 합니다.
...
```powershell
cd C:\aidev\02_supabase-ai-backend\05_backend-mini-service-practice\04_implementation-guide
