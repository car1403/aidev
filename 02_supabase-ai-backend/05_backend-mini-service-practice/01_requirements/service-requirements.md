...
| 제외 항목 | 이유 |
| --- | --- |
| SSE 실시간 스트리밍 | 백엔드/프론트엔드/Supabase 저장을 함께 보아야 하므로 `04_supabase-ai-mini-project`에서 다룹니다. |
| 프론트엔드 화면 | 이번 단원은 API 중심입니다. 화면은 `03_supabase-ai-frontend`와 `04_supabase-ai-mini-project`에서 다룹니다. |
| Docker 배포 | Docker는 `05_llm-agent-orchestration` 이후 본격적으로 사용합니다. |
| Docker Compose, AWS 배포 | `07_multi-agent-service-ops`에서 다룹니다. |
