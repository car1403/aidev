...
이 문서는 `07_backend-service-data-management` 단원에서 계속 참고하는 서비스 데이터 설계 기준입니다.
...
- `06_supabase-db-and-auth`: Supabase 프로젝트, Auth, RLS, Upstash Redis 기본 사용
- `07_backend-service-data-management`: 사용자 프로필, 대화 이력, 서비스 로그, API 엔드포인트 설계
- `08_backend-mini-service-practice`: 위 데이터를 활용한 작은 백엔드 서비스 구현
