...
이 단원에서는 **브라우저나 다른 프로그램이 호출할 수 있는 API 서버**를 직접 만들어 봅니다. 아직 Supabase에 저장하지 않고, 먼저 FastAPI의 기본 구조와 요청/응답 흐름을 익힙니다. 이후 `05_llm-api-integration`에서 AI 모델 API 호출 구조를 연결하고, `06_supabase-db-and-auth`에서 데이터베이스 저장과 인증을 Supabase 기준으로 연결합니다.
...
```powershell
cd C:\aidev\02_supabase-ai-backend\04_fastapi-backend\10_labs\lab01_hello-fastapi
uvicorn solution:app --reload
```
...
```powershell
cd C:\aidev\02_supabase-ai-backend\04_fastapi-backend\10_labs\lab02_path-query-params
uvicorn solution:app --reload
```
...
```powershell
cd C:\aidev\02_supabase-ai-backend\04_fastapi-backend\10_labs\lab03_request-body-validation
uvicorn solution:app --reload
```
...
```powershell
cd C:\aidev\02_supabase-ai-backend\04_fastapi-backend\05_error-handling-and-testing
...
```powershell
cd C:\aidev\02_supabase-ai-backend\04_fastapi-backend\10_labs\lab04_response-model
uvicorn solution:app --reload
```
...
```powershell
cd C:\aidev\02_supabase-ai-backend\04_fastapi-backend\10_labs\lab05_async-external-api
uvicorn solution:app --reload
```
...
```powershell
cd C:\aidev\02_supabase-ai-backend\04_fastapi-backend\10_labs\lab06_in-memory-crud-api
uvicorn solution:app --reload
```
...
```powershell
cd C:\aidev\02_supabase-ai-backend\04_fastapi-backend\10_labs\lab99_mini-api-server
uvicorn solution:app --reload
```
...
```text
Supabase 테이블 저장:
  이후 06_supabase-db-and-auth에서 진행합니다.

실제 LLM API 호출:
  이후 05_llm-api-integration에서 진행합니다.
...
이 단원 이후 `05_llm-api-integration`에서 FastAPI와 AI 모델 API 호출 구조를 연결합니다. 그 다음 `06_supabase-db-and-auth`에서 Supabase 테이블, Auth, RLS, 대화 이력, 서비스 로그 저장 구조를 FastAPI에 연결합니다.

Redis는 `06_supabase-db-and-auth`에서 Upstash Redis를 사용해 캐시와 TTL 개념을 먼저 다룹니다. Docker 기반 Redis 운영과 로컬 PostgreSQL 운영은 `C:\aidev\07_multi-agent-service-ops`에서 본격적으로 다룹니다.
