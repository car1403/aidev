...
```powershell
cd C:\aidev\02_supabase-ai-backend\04_fastapi-backend\01_fastapi-project-setup
uvicorn main:app --reload
```
...
`01_hello-fastapi.py`처럼 하이픈(`-`)이 들어간 파일은 `uvicorn 파일명:app` 방식으로 실행하기 어렵습니다. 실제 프로젝트에서는 `main.py`, `app.py`, `server.py`처럼 단순한 파일명을 권장합니다.
