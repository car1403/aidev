...
앞 단원인 `02_llm-api-integration`에서는 LLM 호출 흐름을 `mock-first -> Gemini SDK 최소 예제 -> Gemini SDK 안내형 예제 -> OpenAI 선택 비교`로 정리했습니다. 따라서 이 챕터의 서비스 로그는 mock 응답과 Gemini SDK 응답을 모두 기록할 수 있는 구조로 설계합니다.
