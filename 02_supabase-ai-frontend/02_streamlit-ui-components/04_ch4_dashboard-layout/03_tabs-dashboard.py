...
st.title("탭 대시보드")  # Streamlit 화면의 가장 큰 제목을 표시합니다.
...
df = pd.DataFrame(  # 딕셔너리 데이터를 행과 열을 가진 DataFrame 표 구조로 변환합니다.
    {
        ...
        "students": [24, 18, 20],  # 이 줄은 예제의 핵심 동작을 단계별로 보여주기 위한 코드입니다.
        "average": [78, 84, 81],  # 이 줄은 예제의 핵심 동작을 단계별로 보여주기 위한 코드입니다.
    }
)

tab_table, tab_chart = st.tabs(["표", "차트"])  # 이 줄은 예제의 핵심 동작을 단계별로 보여주기 위한 코드입니다.
