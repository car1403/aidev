...
이 폴더에서는 화면을 만들고, 백엔드와 데이터를 직접 연결하는 흐름을 연습합니다. Supabase 프로젝트, 테이블, 인증, 대화 기록 저장은 `01_supabase-ai-backend`에서 준비하고, 이 과정에서는 그 백엔드 API를 호출하는 방식으로 진행합니다.
...
작성한 Streamlit 코드를 AI와 함께 점검합니다. 오류 메시지 읽기, UI 개선, API 호출 문제 분석, 코드 리팩토링을 연습합니다.
