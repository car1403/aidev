...
수업 참여자는 이 폴더에서 화면을 만들고, 백엔드와 데이터를 직접 연결하는 흐름을 연습합니다. Supabase 프로젝트, 테이블, 인증, 대화 기록 저장은 `01_supabase-ai-backend`에서 준비하고, 이 과정에서는 그 백엔드 API를 호출하는 방식으로 진행합니다.
...
- React는 필수 과정이 아닙니다. Streamlit 수업을 기본으로 진행하고, 진도와 난이도과 수업 시간에 따라 React 기반 UI 구조는 선택 확장으로 소개합니다.
- 최종 배포 실습은 이 과정에서 하지 않고, 배포 전 점검 항목만 확인합니다. Docker, AWS, GitHub Actions 배포는 `06_multi-agent-service-ops`에서 다룹니다.

## 학습 목표

이 과정을 마치면 수업 참여자는 다음 내용을 설명하고 직접 구현할 수 있어야 합니다.
...
이 과정의 기본 UI 도구는 Streamlit입니다. React는 필수 실습이 아니라 수업 참여자의 수준과 수업 진도에 따라 선택적으로 소개합니다.
...
수업 참여자가 작성한 Streamlit 코드를 AI와 함께 점검합니다. 오류 메시지 읽기, UI 개선, API 호출 문제 분석, 코드 리팩토링을 연습합니다.
...
수업 참여자는 오류가 발생했을 때 아래 순서로 확인합니다.
...
| React 기반 AI 서비스 UI 구조 이해 | 선택 확장 | 선택 | 진도와 난이도과 수업 진도에 따라 비교 개념으로만 다룹니다. 기본 구현은 Streamlit입니다. |
...
수업 진행에서는 `01_supabase-ai-backend`에서 Supabase 설정과 API 실행을 먼저 보여준 뒤, 이 과정에서 같은 API를 Streamlit 화면에서 호출하도록 수업을 연결하면 좋습니다.

수업 참여자에게는 다음 기준을 반복해서 안내합니다.
