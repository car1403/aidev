...
Server-Sent Events(SSE) 기반 실시간 AI 응답 표시는 `03_supabase-ai-mini-project`에서 FastAPI SSE 엔드포인트와 함께 통합 실습으로 다룹니다.
...
이 과정은 Streamlit으로 AI 서비스 화면을 만들고, `01_supabase-ai-backend`의 Supabase 기반 API와 연결하는 프론트엔드 수업입니다.

학생은 이 폴더에서 화면을 만들고, 백엔드와 데이터를 직접 연결하는 흐름을 연습합니다. Supabase 프로젝트, 테이블, 인증, 대화 기록 저장은 `01_supabase-ai-backend`에서 준비하고, 이 과정에서는 그 백엔드 API를 호출하는 방식으로 진행합니다.

01~03 과정의 AI 응답 생성 API는 Gemini를 기본으로 사용합니다. 프론트엔드는 Gemini API key를 직접 다루지 않고, `01_supabase-ai-backend` 또는 `03_supabase-ai-mini-project`의 FastAPI 백엔드를 호출합니다. 기존 OpenAI 예제는 선택/비교 실습용으로 유지합니다.

Docker, Docker Compose, AWS 배포, 운영 자동화는 이 과정에서 다루지 않습니다. 해당 내용은 뒤의 `06_multi-agent-service-ops` 과정에서 별도로 학습합니다.

## 수업 기준

- 프론트엔드는 Streamlit으로 작성합니다.
- 데이터 저장소는 Supabase를 기준으로 이해합니다.
- 프론트엔드는 Supabase DB에 직접 접속하지 않고 `01_supabase-ai-backend`의 FastAPI API를 호출합니다.
- `.env`에는 프론트엔드가 호출할 백엔드 주소인 `API_BASE_URL`만 둡니다.
- Supabase `service_role` 키는 절대 프론트엔드 폴더에 두지 않습니다.
- Gemini 또는 OpenAI 같은 LLM API key도 프론트엔드 폴더에 두지 않습니다. 실제 AI API 호출은 백엔드에서 처리합니다.
- Docker 실행 방식은 사용하지 않고, 로컬 Python 가상환경과 Streamlit 실행 방식으로 진행합니다.
- React는 필수 과정이 아닙니다. Streamlit 수업을 기본으로 진행하고, 학생 수준과 수업 시간에 따라 React 기반 UI 구조는 선택 확장으로 소개합니다.
- 최종 배포 실습은 이 과정에서 하지 않고, 배포 전 점검 항목만 확인합니다. Docker, AWS, GitHub Actions 배포는 `06_multi-agent-service-ops`에서 다룹니다.

## 학습 목표

이 과정을 마치면 학생은 다음 내용을 설명하고 직접 구현할 수 있어야 합니다.
...
-> Streamlit 화면
-> httpx API 요청
-> 01_supabase-ai-backend FastAPI 서버
...
2. `02_supabase-ai-frontend` 폴더에 `.venv`를 만듭니다.
...
8. `03_api-integration`부터는 `01_supabase-ai-backend` 서버를 먼저 실행한 뒤 실습합니다.

이 과정에서는 각 하위 폴더마다 `.venv`를 따로 만들지 않습니다. `02_supabase-ai-frontend` 최상위의 `.venv` 하나를 모든 단원에서 함께 사용합니다.
...
`httpx`로 백엔드 API를 호출합니다. 이 단원부터는 `01_supabase-ai-backend`의 Supabase 기반 FastAPI 서버를 우선 연결 대상으로 사용합니다. 폴더 안의 샘플 백엔드는 백엔드가 아직 준비되지 않았을 때 개념 확인용으로만 사용합니다.

### 04_ai-chatbot-interface

AI 챗봇 화면을 만듭니다. 사용자 메시지 입력, assistant 응답 표시, 대화 이력 UI, `/api/chat` 같은 백엔드 API 호출 구조를 학습합니다.
...
이 과정의 기본 UI 도구는 Streamlit입니다. React는 필수 실습이 아니라 학생의 수준과 수업 진도에 따라 선택적으로 소개합니다.
...
학생이 작성한 Streamlit 코드를 AI와 함께 점검합니다. 오류 메시지 읽기, UI 개선, API 호출 문제 분석, 코드 리팩토링을 연습합니다.
...
그래서 `02_supabase-ai-frontend`에서는 로컬 Python 가상환경과 Streamlit 중심으로 실습합니다. Docker, Docker Compose, AWS, GitHub Actions, 서비스 운영, 모니터링은 `06_multi-agent-service-ops`에서 단계적으로 다룹니다.

## 배포 전 점검 범위

이 과정에서는 초보자용 무료 배포 흐름도 함께 안내합니다. 다만 Docker, AWS, GitHub Actions처럼 운영 난도가 높은 배포 자동화는 `06_multi-agent-service-ops`에서 본격적으로 다룹니다.
...
6. 오류가 발생했을 때 학생 또는 사용자가 이해할 수 있는 안내 메시지가 보이는가?
```

초보자용 무료 배포 실습은 다음 문서를 기준으로 진행합니다.

```text
99_final-frontend-project/docs/free-deployment-guide.md
```
...
운영 자동화, Docker Compose, AWS, GitHub Actions, 모니터링은 `06_multi-agent-service-ops`에서 더 깊게 다룹니다.
...
강사는 먼저 화면이 단독으로 실행되는 예제를 보여준 뒤, 같은 화면이 백엔드 API를 호출하도록 바뀌는 과정을 보여주면 좋습니다.

학생은 오류가 발생했을 때 아래 순서로 확인합니다.
...
6. Supabase 설정은 `01_supabase-ai-backend`에서 완료했는가?

## 커리큘럼 점검

이미지의 `AI 인터페이스 및 UX 설계` 영역을 기준으로 `02_supabase-ai-frontend`에 반영된 내용을 점검한 표입니다.

이 과정은 전체적으로 Supabase 기반 백엔드와 연결하는 프론트엔드 수업입니다. 프론트엔드에서 Supabase DB를 직접 다루기보다, `01_supabase-ai-backend`의 FastAPI API를 호출하는 흐름을 기준으로 합니다.

요약하면 다음과 같습니다.

- Streamlit 기반 UI, 챗봇 UI, API 연동, 프롬프트 입력, 로그인 상태, 사용자별 대화 이력, 캐시는 현재 구조에 포함되어 있습니다.
- Supabase Auth, RLS, 대화 테이블, 서비스 로그는 `01_supabase-ai-backend`에서 구현하고, 이 과정에서는 해당 API를 호출하는 화면을 만듭니다.
- 실시간 응답 표시는 이 과정에서 `st.spinner`, `st.status`, `st.empty` 기반 기초를 다루고, SSE 기반 스트리밍은 `03_supabase-ai-mini-project`에서 통합 실습으로 다룹니다.
- 사용 로그 화면과 통합 점검은 `05_state-session-and-data/06_ch6_service-log-and-integration-check`에서 다룹니다.
- 무료 서비스 기반 최종 배포 흐름은 초보자용 가이드로 다룹니다. Docker/AWS/GitHub Actions 기반 운영 자동화는 `06_multi-agent-service-ops`에서 다룹니다.
...
| React 기반 AI 서비스 UI 구조 이해 | 선택 확장 | 선택 | 훈련생 수준과 수업 진도에 따라 비교 개념으로만 다룹니다. 기본 구현은 Streamlit입니다. |
...
| AI 모델 API 연동 | `04_ai-chatbot-interface/04_ch4_mock-chat-service-interface` | 부분 포함 | 기본 구조는 포함되어 있습니다. 실제 수업에서는 `01_supabase-ai-backend`의 `/api/chat` 또는 수업용 chat API와 연결합니다. |
| 프롬프트 작성 | `04_ai-chatbot-interface/02_ch2_prompt-input-and-response` | 포함 | 사용자 질문과 응답 템플릿을 학습합니다. |
| 사용자 질문 응답 기능 구현 | `04_ai-chatbot-interface` | 포함 | 화면 중심 구현 후 백엔드 API 호출로 확장합니다. |
| 로그인 상태 유지 | `05_state-session-and-data/03_ch3_auth-token-and-login-state` | 포함 | Supabase Auth는 백엔드에서 처리하고, 프론트엔드는 token 저장과 화면 분기를 담당합니다. |
| 사용자별 대화 기록 조회 및 관리 | `05_state-session-and-data/04_ch4_user-data-and-conversation-history` | 포함 | Supabase 대화 테이블 API를 호출하는 화면으로 연결합니다. |
| 이전 대화를 활용한 연속 대화 기능 | `04_ai-chatbot-interface/03_ch3_conversation-history` | 포함 | 프론트엔드 대화 이력 구조를 먼저 만들고, 이후 백엔드 저장 이력과 연결합니다. |
| 프롬프트 설계 및 응답 품질 개선 | `04_ai-chatbot-interface/02_ch2_prompt-input-and-response`, assignments | 포함 | 과제에서 질문 개선, 응답 검토, UI 메시지 품질 개선을 다룹니다. |
...
| 사용자 인터랙션 및 사용 로그 기록 | `05_state-session-and-data/06_ch6_service-log-and-integration-check` | 보강 | 백엔드의 service log API 또는 Supabase log table을 조회하는 화면으로 다룹니다. |
| 개인화 AI 챗봇 서비스 통합 구현 | `99_final-frontend-project` | 포함 | Streamlit 화면, FastAPI 호출, 사용자별 대화 이력, 서비스 로그 조회를 하나의 간단한 예제로 통합합니다. |
| 서비스 로그 수집 | `99_final-frontend-project`, `05_state-session-and-data/06_ch6_service-log-and-integration-check` | 보강 | 수집 자체는 백엔드 영역이며, 프론트엔드는 로그 조회와 상태 점검 화면을 담당합니다. |
| 최종 배포 실습 | `99_final-frontend-project/docs/free-deployment-guide.md` | 보강 | FastAPI는 Render, Redis는 Upstash, Streamlit은 Streamlit Community Cloud에 배포하는 초보자용 흐름을 안내합니다. Docker/AWS/GitHub Actions는 06에서 진행합니다. |

## 강의 운영 메모

강사는 `01_supabase-ai-backend`에서 Supabase 설정과 API 실행을 먼저 보여준 뒤, 이 과정에서 같은 API를 Streamlit 화면에서 호출하도록 수업을 연결하면 좋습니다.

학생에게는 다음 기준을 반복해서 안내합니다.
...
- 무료 배포 서비스 기반의 간단한 최종 배포는 02 마지막에서 안내하고, 운영 자동화는 06 과정에서 배웁니다.
