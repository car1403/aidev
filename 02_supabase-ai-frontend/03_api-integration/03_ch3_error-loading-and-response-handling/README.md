...
이 챕터의 실시간 표시는 SSE 스트리밍이 아니라, 사용자가 기다리는 동안 화면이 멈춘 것처럼 보이지 않게 만드는 기초 UI 처리입니다. `st.spinner`, `st.status`, 상태 메시지, 오류 안내 문구를 먼저 익힌 뒤, SSE 기반 응답 스트리밍은 `03_supabase-ai-mini-project`에서 통합 실습으로 다룹니다.
...
```powershell
streamlit run.\03_api-integration\03_ch3_error-loading-and-response-handling\02_timeout-error-handling.py
```
