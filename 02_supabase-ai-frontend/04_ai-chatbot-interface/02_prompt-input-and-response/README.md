...
이 챕터에서는 프롬프트 문장 자체와 화면 표시 흐름을 먼저 다룹니다. 실제 LLM API 호출, 토큰 비용, 모델 파라미터는 `01_supabase-ai-backend`에서 이미 학습한 내용을 바탕으로 하고, 여기서는 응답을 화면에 잘 보여주는 역할에 집중합니다.
...
streamlit run .\04_ai-chatbot-interface\02_prompt-input-and-response\02_mock-ai-response.py
```
...
이 폴더의 mock 응답 함수는 이후 Gemini SDK 호출 함수나 백엔드 chat API 호출 함수로 바꿀 수 있습니다.
