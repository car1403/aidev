...
`01_supabase-ai-backend`에서 Python과 FastAPI를 통해 “서버가 데이터를 처리하는 방식”을 배웠다면, 이 단원에서는 사용자가 실제로 보는 화면을 Python으로 구성합니다. 아직 Supabase, Gemini SDK, 로그인, SSE 스트리밍을 직접 연결하지는 않습니다. 먼저 화면 출력, 입력, 레이아웃을 충분히 익힌 뒤 다음 단원에서 API 호출과 챗봇 UI로 확장합니다.
...
`02_supabase-ai-frontend` 최상위 폴더에서 가상환경을 활성화한 뒤 예제 파일을 Streamlit으로 실행합니다.
...
패키지 설치가 필요하면 `02_supabase-ai-frontend`의 `requirements.txt`를 기준으로 설치합니다.
