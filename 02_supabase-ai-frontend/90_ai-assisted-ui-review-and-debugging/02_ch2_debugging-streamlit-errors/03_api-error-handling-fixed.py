...
st.title("디버깅 예제: API 오류 처리")  # Streamlit 화면의 가장 큰 제목을 표시합니다.

if st.button("상태 확인"):  # 조건식이 True일 때만 아래 들여쓰기 블록을 실행합니다.
    try:  # 실패할 수 있는 코드를 실행하고, 오류가 나면 except 블록에서 처리합니다.
        response = httpx.get(f"{API_BASE_URL}/health", timeout=3.0)  # GET 요청의 응답을 response 변수에 저장해 상태 코드와 JSON 데이터를 확인합니다.
        ...
        st.error("FastAPI 서버가 실행 중인지 확인하세요.")  # 오류 상황을 사용자에게 명확히 보여줍니다.
        ...
        st.error(f"API 상태 코드 오류: {error.response.status_code}")  # 오류 상황을 사용자에게 명확히 보여줍니다.
