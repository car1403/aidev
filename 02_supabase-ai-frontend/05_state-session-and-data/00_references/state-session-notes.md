...
`04_ai-chatbot-interface`에서는 챗봇 화면을 먼저 만들고, mock 응답과 간단한 대화 이력 미리보기를 확인합니다.

`05_state-session-and-data`에서는 같은 화면 흐름을 실제 서비스 상태 관리로 확장합니다. 로그인 token을 저장하고, Authorization header를 붙여 백엔드 API를 호출하고, 사용자별 대화 이력과 서비스 로그를 조회합니다.
