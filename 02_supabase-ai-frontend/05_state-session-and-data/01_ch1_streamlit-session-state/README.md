...
Streamlit은 버튼 클릭이나 입력 변경이 있을 때마다 Python 파일을 다시 실행합니다. 따라서 화면에서 계속 유지해야 하는 값은 `st.session_state`에 저장해야 합니다.
...
- 초기화 버튼을 누르면 상태가 비워지는가?
- 상태 초기화 코드가 사용 전에 실행되는가?

## 흔한 오류

- `st.session_state.count`를 초기화하기 전에 사용하는 경우 오류가 발생할 수 있습니다.
