...
로그인된 사용자의 데이터와 대화 이력을 조회하고 저장하는 방법을 학습합니다.

이 챕터는 Supabase 테이블을 프론트엔드에서 직접 조회하는 수업이 아닙니다. 백엔드 API가 Supabase와 통신하고, Streamlit은 Authorization header를 붙여 백엔드 API를 호출합니다.
...
C:\aidev\01_supabase-ai-backend\07_backend-service-data-management\02_conversation-history
```
...
- `Bearer sample-access-token` 또는 실제 로그인 token 형식의 인증 헤더가 사용되는가?
...
- Docker와 배포 환경은 `06_multi-agent-service-ops`에서 따로 학습합니다.
