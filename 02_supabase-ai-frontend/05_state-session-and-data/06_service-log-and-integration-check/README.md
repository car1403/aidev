...
이 단원은 로그를 프론트엔드에서 직접 저장하는 수업이 아닙니다. 실제 서비스 로그 저장은 `01_supabase-ai-backend`와 Supabase가 담당합니다. Streamlit은 백엔드 API를 호출해 최근 로그를 조회하고, 현재 UI-백엔드-데이터 연결 상태를 눈으로 확인할 수 있게 보여줍니다.
...
.\.venv\Scripts\Activate.ps1
streamlit run .\05_state-session-and-data\06_service-log-and-integration-check\01_service-log-dashboard.py
```
...
Docker, Docker Compose, AWS, GitHub Actions를 사용하는 운영 자동화와 모니터링은 `06_multi-agent-service-ops`에서 더 깊게 다룹니다.
