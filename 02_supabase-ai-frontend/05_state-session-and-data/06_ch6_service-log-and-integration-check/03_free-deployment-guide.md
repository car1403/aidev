...
이 문서는 `02_supabase-ai-frontend`의 마지막 통합 실습에서 사용합니다. Docker, Docker Compose, AWS, GitHub Actions 기반 운영 자동화는 `06_multi-agent-service-ops`에서 더 깊게 다룹니다.

## 0. 먼저 알아둘 점

무료 서비스의 정책, 제한, 화면 메뉴 이름은 바뀔 수 있습니다. 수업 당일에는 각 서비스의 공식 화면에서 현재 무료 범위와 과금 조건을 다시 확인합니다.

이 실습에서 중요한 것은 “완벽한 운영 배포”가 아니라, 학생이 다음 흐름을 직접 경험하는 것입니다.
...
VS Code 설치
Git 설치
01_supabase-ai-backend 실행 확인
02_supabase-ai-frontend 실행 확인
...
```text
C:\aidev\01_supabase-ai-backend\requirements.txt
C:\aidev\01_supabase-ai-backend\.env.example
C:\aidev\02_supabase-ai-frontend\requirements.txt
C:\aidev\02_supabase-ai-frontend\.env.example
```
...
- 01~03 과정의 기본 AI API는 Gemini입니다. OpenAI API key는 선택/비교 실습을 할 때만 등록합니다.
...
Render 공식 FastAPI 예시에서도 Python Web Service의 Build Command는 `pip install -r requirements.txt`, Start Command는 `uvicorn main:app --host 0.0.0.0 --port $PORT` 형태를 사용합니다.
...
10. 배포된 Streamlit 화면에서 Render 백엔드 호출 확인
11. 학생별 오류 상황 공유 및 해결
```
