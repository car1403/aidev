...
이 챕터는 로그를 프론트엔드에서 직접 저장하는 수업이 아닙니다. 실제 서비스 로그 저장은 `01_supabase-ai-backend`와 Supabase가 담당합니다. Streamlit은 백엔드 API를 호출해 최근 로그를 조회하고, 현재 UI-백엔드-데이터 연결 상태를 학생이 눈으로 확인할 수 있게 보여줍니다.
...
- 실제 배포 전에 프론트엔드가 확인해야 할 항목을 체크리스트로 정리할 수 있다.
...
실제 수업에서는 `01_supabase-ai-backend`의 서비스 로그 API와 연결합니다.

```text
C:\aidev\01_supabase-ai-backend\06_supabase-db-and-auth\05_ch5_conversation-history-and-service-logs
C:\aidev\01_supabase-ai-backend\07_backend-service-data-management\03_service-logs
```
...
- 실제 배포 전에 확인할 항목을 학생이 설명할 수 있는가?

## 최종 배포 실습

이 챕터에서는 초보자가 따라 할 수 있는 무료 배포 서비스 기반의 최종 배포 흐름을 안내합니다.
...
Docker, Docker Compose, AWS, GitHub Actions를 활용한 운영 자동화와 모니터링은 `06_multi-agent-service-ops`에서 더 깊게 다룹니다.
