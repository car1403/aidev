...
이 단원은 Supabase Auth를 프론트엔드에서 직접 호출하는 수업이 아닙니다. Supabase Auth는 `01_supabase-ai-backend`에서 처리하고, Streamlit은 백엔드 로그인 API를 호출한 뒤 응답으로 받은 token을 `st.session_state`에 저장합니다.
