...
최종 실습을 마친 뒤 수업 참여자가 스스로 확인할 체크리스트입니다.
...
- [ ] API key와 token을 코드에 직접 적지 않았다.
- [ ] Upstash token은 백엔드 환경변수에만 등록했다.
- [ ] Supabase service role key는 프론트엔드에 넣지 않는다는 기준을 이해했다.

## 03 미니 프로젝트 연결

- [ ] 메모리 저장을 Supabase 저장으로 바꾸어야 하는 이유를 설명할 수 있다.
- [ ] 대화 이력, 서비스 로그, 피드백 테이블이 왜 필요한지 설명할 수 있다.
- [ ] SSE 스트리밍이 03에서 다루어지는 이유를 설명할 수 있다.
