...
이 문서는 초보자가 `99_final-frontend-project` 예제를 무료 배포 서비스에 올려보는 과정을 설명합니다.
...
Streamlit은 `API_BASE_URL`을 보고 백엔드 주소를 결정합니다.
...
배포 후:
API_BASE_URL=https://내서비스이름.onrender.com
```
...
- 배포 서비스의 환경변수 또는 Secrets 메뉴에 값을 등록합니다.

## 2. GitHub 저장소 준비

초보자에게는 전체 `C:\aidev`를 그대로 올리는 것보다, 배포할 예제만 별도 저장소로 정리하는 방식을 권장합니다.
...
```text
C:\aidev\02_supabase-ai-frontend\99_final-frontend-project\backend
C:\aidev\02_supabase-ai-frontend\99_final-frontend-project\frontend
```
...
이 예제에서는 Upstash가 필수는 아닙니다. 다만 배포 흐름을 익히기 위해 생성해 봅니다.
...
6. Free plan을 선택합니다.
7. 생성 후 상세 화면에서 REST API 정보를 확인합니다.
...
### 4-2. Build Command 입력

```text
pip install -r requirements.txt
```

### 4-3. Start Command 입력
...
```text
https://내서비스이름.onrender.com/health
```
...
저장소 구조를 다르게 만들었다면 실제 `app.py` 위치에 맞게 입력합니다.
