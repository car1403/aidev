...
st.set_page_config(
    page_title="Final Frontend Project",
    layout="wide",
)

st.title("개인화 AI 챗봇 서비스")
st.caption("Streamlit 프론트엔드가 FastAPI 백엔드를 호출하는 최종 예제입니다.")
...
if ok:
    ...
    st.error("백엔드 연결 실패")
    st.write(result)
    st.warning("FastAPI 백엔드를 먼저 실행했는지, API_BASE_URL이 맞는지 확인하세요.")
...
st.write(
    ...
    대시보드 산출물로 확장됩니다. 02에서는 프론트엔드 연결 감각을 익히고,
    03에서는 팀 프로젝트 형태로 구조를 완성합니다.
    """
)
