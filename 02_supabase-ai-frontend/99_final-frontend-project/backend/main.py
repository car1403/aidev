...
app = FastAPI(
    ...
    description="02 프론트엔드 최종 예제를 위한 간단한 FastAPI 백엔드입니다.",
    version="1.0.0",
)

# Streamlit Community Cloud처럼 다른 주소에서 실행되는 프론트엔드가
# 이 백엔드 API를 호출할 수 있도록 CORS를 열어 둡니다.
# 실제 서비스에서는 허용할 프론트엔드 도메인을 명확히 제한하는 것이 좋습니다.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# 이 최종 예제는 02 과정의 프론트엔드 연결 감각을 익히기 위한 축소판입니다.
# 별도 데이터베이스 없이 메모리에 대화와 로그를 저장합니다.
# 서버를 재시작하면 아래 목록의 데이터는 사라집니다.
# 03_supabase-ai-mini-project에서는 이 부분을 Supabase 테이블 저장으로 확장합니다.
CHAT_HISTORY: list[dict[str, Any]] = []
SERVICE_LOGS: list[dict[str, Any]] = []


class ChatRequest(BaseModel):
    user_name: str = Field(..., min_length=1, description="질문을 보낸 사용자 이름")
...
def utc_now() -> str:
    """프론트엔드와 로그 화면에서 읽기 쉬운 UTC 시간 문자열을 만듭니다."""
    return datetime.now(timezone.utc).isoformat()


def build_demo_answer(user_name: str, message: str) -> str:
    """실제 LLM API 대신 수업용 mock 응답을 만듭니다.

    02 과정의 목표는 Streamlit 화면이 FastAPI 백엔드를 호출하고,
    응답과 로그를 화면에 표시하는 전체 흐름을 익히는 것입니다.
    Gemini 실제 호출과 Supabase 저장은 01, 03 과정에서 더 깊게 다룹니다.
    """
    return (
        f"{user_name}님의 질문을 확인했습니다. "
        f"'{message}'에 대해서는 먼저 요구사항을 작게 나누고, "
        "입력값 검증, API 응답 처리, 화면 표시, 서비스 로그 기록 순서로 점검해 보세요."
    )
...
async def save_log_to_upstash(log: dict[str, Any]) -> None:
    ...
    `UPSTASH_REDIS_REST_URL`과 `UPSTASH_REDIS_REST_TOKEN`이 비어 있으면
    ...
    # Upstash REST API는 Redis 명령을 URL 경로로 전달할 수 있습니다.
    # 여기서는 최신 로그를 빠르게 볼 수 있도록 list에 push합니다.
    ...
    try:
        ...
        # 로그 저장 실패가 챗봇 응답 실패로 이어지지 않도록 조용히 넘어갑니다.
        # 실제 서비스에서는 이 실패도 별도 모니터링 대상으로 기록합니다.
        return
...
def health() -> dict[str, Any]:
    """로컬 실행과 Render 배포 후 가장 먼저 확인할 상태 점검 API입니다."""
    return {
        "ok": True,
...
async def create_chat(request: ChatRequest) -> ChatResponse:
    """사용자 질문을 받아 mock AI 응답을 만들고 대화 이력과 로그를 저장합니다."""
