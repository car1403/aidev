...
`01_supabase-ai-backend`와 `02_supabase-ai-frontend`에서 배운 내용을 연결해 Supabase, FastAPI, Streamlit으로 로그를 저장, 조회, 시각화하는 작은 서비스를 완성합니다.
...
Docker, Docker Compose, AWS 배포, GitHub Actions 자동화는 `06_multi-agent-service-ops`에서 학습합니다. 03 과정의 배포는 선택 시연이며, 기본 제출 기준은 로컬에서 정상 동작하는 것입니다.

## 필수 산출물

팀은 아래 4가지 산출물을 제출합니다.

| 산출물 | 설명 |
| --- | --- |
| API 설계 문서 | FastAPI 엔드포인트, HTTP Method, Request/Response, 오류 응답 규칙 |
| 화면 설계서 | 메인, 상세, 입력, 설정, 오류 화면의 와이어프레임과 사용자 액션 흐름 |
| 데이터베이스 설계서 | Supabase 테이블, 컬럼, 관계, 제약조건, ERD, RLS 정책 |
