...
PROJECT_ENV = Path(__file__).resolve().parents[3] / ".env"  # 03_supabase-ai-mini-project 최상위 .env 파일 경로입니다.
...
SUPABASE_SERVICE_ROLE_KEY = os.getenv("SUPABASE_SERVICE_ROLE_KEY")  # 서버에서만 사용할 수 있는 강한 권한의 key입니다. 프론트엔드에 노출하면 안 됩니다.
...
def get_supabase_client():  # Supabase 연결 객체를 만드는 함수입니다. 여러 API에서 같은 연결 방식을 재사용합니다.
    supabase_key = SUPABASE_SERVICE_ROLE_KEY or SUPABASE_ANON_KEY  # 백엔드에서는 service role key를 우선 사용하고, 없으면 anon key로 기본 연결을 확인합니다.
