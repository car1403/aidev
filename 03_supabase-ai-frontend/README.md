...
- 필수: Streamlit 화면 구성, 입력/버튼/표/차트, FastAPI 백엔드 호출, 로딩/오류 상태 처리를 실습합니다.
- 선택: React 구조 비교, 프론트엔드 Gemini SDK 직접 호출 예제, 무료 배포 확장은 진도에 따라 다룹니다.
- 다음 과정으로 넘어가기 전: 프론트엔드가 `API_BASE_URL`을 통해 백엔드를 호출하고, service role key를 프론트엔드에 두지 않는 이유를 설명할 수 있어야 합니다.

## 막혔을 때 바로 보기

| 막히는 지점 | 확인 문서 |
| --- | --- |
| Streamlit 실행 오류 | [Streamlit 오류 해결](../00_course-guide/08_environment-and-troubleshooting/05_streamlit-errors.md), [SETUP.md](./SETUP.md) |
| 백엔드 연결 실패 | [03 Frontend SETUP](./SETUP.md), [FastAPI 오류 해결](../00_course-guide/08_environment-and-troubleshooting/04_fastapi-errors.md) |
| `API_BASE_URL` 또는 `.env` 위치가 헷갈림 | [SETUP.md](./SETUP.md), [환경과 오류 해결 허브](../00_course-guide/08_environment-and-troubleshooting/README.md) |
| service role key를 프론트엔드에 넣어도 되는지 헷갈림 | [Supabase 오류 해결](../00_course-guide/08_environment-and-troubleshooting/06_supabase-errors.md), [02 Backend SETUP](../02_supabase-ai-backend/SETUP.md) |
