...
`08_multi-agent-service-mini-project`는 `07_multi-agent-service-ops`에서 학습한 Multi-Agent 협업, Docker Compose, 서비스 운영, Auto Healing, 보안 가드레일, 관측성(Observability)을 하나의 미니 프로젝트로 묶어 구현하는 과정입니다.
...
08은 새로운 이론을 많이 추가하는 과정이 아닙니다. 07에서 배운 내용을 실제 프로젝트 산출물로 정리하고, Docker Compose 기반 서비스로 실행하며, 최종 발표와 제출이 가능한 형태로 다듬는 단계입니다.
