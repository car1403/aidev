...
```powershell
cd C:\aidev\01_supabase-ai-backend\04_fastapi-backend\05_error-handling-and-testing
...
Docker 실행:
  이 과정에서는 사용하지 않습니다. Docker 기반 운영은 C:\aidev\06_multi-agent-service-ops에서 다룹니다.

AWS 배포와 CI/CD:
  이 과정에서는 다루지 않고, 06_multi-agent-service-ops에서 진행합니다.
```
