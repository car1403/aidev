...
```powershell
cd C:\aidev\01_supabase-ai-backend\04_fastapi-backend\04_async-and-external-api
...
이번 단원에서는 스트리밍의 개념만 확인합니다. **Server-Sent Events(SSE) 기반 AI 응답 스트리밍**은 `03_supabase-ai-mini-project`에서 백엔드, 프론트엔드, Supabase 저장 흐름과 함께 통합 실습으로 다룹니다.
...
```text
실제 Gemini API 호출:
  05_llm-api-integration에서 진행합니다.

Supabase 저장:
  06_supabase-db-and-auth에서 진행합니다.

SSE 기반 AI 응답 스트리밍 통합:
  03_supabase-ai-mini-project에서 진행합니다.

Docker 실행:
  04_llm-agent-orchestration부터 Docker Desktop 기반으로 시작하고,
  Docker Compose와 AWS 배포는 06_multi-agent-service-ops에서 다룹니다.
```
