...
앞 단원에서는 LLM 호출을 `mock-first -> Gemini SDK 기본 구현 -> REST 보충 -> OpenAI 선택 비교` 흐름으로 정리했습니다. 이 실습의 예제는 비용 없이 실행할 수 있도록 mock 응답을 저장하지만, 같은 저장 구조는 Gemini SDK 응답 저장에도 그대로 사용할 수 있습니다.
