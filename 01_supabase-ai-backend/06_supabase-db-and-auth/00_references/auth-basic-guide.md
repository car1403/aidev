...
Supabase 과정에서는 Supabase Auth와 RLS를 사용해 이 두 가지를 나누어 이해합니다.
...
- 사용자별 데이터는 현재 사용자 ID로 필터링해야 합니다.
- RLS 정책을 켜면 데이터 접근 규칙이 테이블 단위로 강제됩니다.

## 현재 과정의 학습 범위

이 단원에서는 인증의 전체 개념과 RLS의 필요성을 먼저 이해합니다. 실제 서비스 수준의 보안 심화, 권한 감사, 운영 보안 정책은 이후 과정에서 더 깊게 다룹니다.
