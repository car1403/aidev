...
`01_supabase-ai-backend`는 Supabase 중심으로 진행합니다. Redis는 Docker로 직접 실행하지 않고, Upstash Redis를 사용해 캐시, TTL, 임시 세션 상태, 요청 횟수 제한을 가볍게 실습합니다. Docker, Docker Compose, 로컬 PostgreSQL/Redis 운영은 `C:\aidev\06_multi-agent-service-ops`에서 본격적으로 학습합니다.

## 학습 목표

- Supabase 프로젝트를 만들고 API URL/key를 설정합니다.
- Supabase Table Editor 또는 SQL Editor로 테이블을 설계합니다.
...
- Upstash Redis로 TTL 기반 캐시와 요청 횟수 제한을 실습합니다.

## 이 단원에서 만드는 것

수업 중에는 다음 결과물을 차례대로 만듭니다.
...
```text
06_supabase-db-and-auth/00_references/supabase-schema.sql
```

Supabase Dashboard의 SQL Editor에서 실행합니다.

초보자는 먼저 `learning_notes` 테이블 하나로 시작하고, 이후 `conversations`, `messages`, `service_logs`로 확장합니다.
...
초보자가 가장 자주 헷갈리는 부분은 "어떤 데이터는 Supabase에 넣고, 어떤 데이터는 Redis에 넣어야 하는가"입니다.
...
초보자 수업에서는 이 폴더를 먼저 사용하지 않습니다.

Docker로 PostgreSQL/Redis를 직접 실행하고 운영하는 방식은 아래 과정에서 다룹니다.
