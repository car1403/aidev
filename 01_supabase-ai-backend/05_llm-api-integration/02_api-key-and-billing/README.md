...
`01_llm-api-concepts`에서는 실제 API 호출 없이 message 구조와 파라미터 개념을 보았습니다. 이번 단원에서도 실제 API 호출은 하지 않고, `.env`에 key가 있는지 안전하게 확인하는 단계까지만 진행합니다.
...
```text
Gemini
  01~03 과정의 기본 LLM 실습 provider입니다.
...
| `01_check_llm_env.py` | `.env`에서 LLM 관련 환경변수 설정 여부 확인 |
| `02_safe_config_loader.py` | 실제 key를 출력하지 않고 설정 객체로 안전하게 읽는 예제 |
...
5. C:\aidev\01_supabase-ai-backend\.env 파일에 저장합니다.
```
...
`C:\aidev\01_supabase-ai-backend\.env` 파일에 아래 형식으로 작성합니다.
...
`C:\aidev\01_supabase-ai-backend\.gitignore` 또는 상위 `.gitignore`에 아래 항목이 있는지 확인합니다.
