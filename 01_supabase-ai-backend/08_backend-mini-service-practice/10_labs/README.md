...
이 폴더는 `08_backend-mini-service-practice`의 실습 안내를 담고 있습니다.
...
LLM 답변 생성은 `05_llm-api-integration`의 변경 방향에 맞춰 `mock-first -> Gemini SDK 기본 구현 -> REST 보충 -> OpenAI 선택 비교` 흐름으로 바라봅니다. 이 단원에서는 실제 SDK 호출보다 저장 구조와 로그 구조를 먼저 완성합니다.
...
실습은 `01_supabase-ai-backend`의 `.venv`를 사용합니다.
