...
```text
C:\aidev\01_supabase-ai-backend\README.md 파일 하나만 열기
```

올바르게 연 예시:

```text
C:\aidev\01_supabase-ai-backend 폴더 전체 열기
```
...
| Initialize Repository | 현재 폴더가 Git 저장소가 아닙니다. | 강사 안내 후 누릅니다. |
...
초보자 수업에서는 보통 `Private` 저장소를 권장합니다.
...
초보자는 수업 중 강사의 안내 없이 브랜치를 무리하게 바꾸지 않습니다.
...
VS Code에서 잘못된 폴더를 열었을 수 있습니다. `C:\aidev\01_supabase-ai-backend` 폴더를 열었는지 확인합니다.
...
```text
1. C:\aidev\01_supabase-ai-backend 폴더를 VS Code로 연다.
