...
이 단원의 `.env`는 `C:\aidev\04_llm-agent-orchestration\02_advanced-prompting-and-reasoning\.env` 파일입니다. 01 단원의 `.env`를 자동으로 읽지 않으므로, 이 단원에서도 `.env.example`을 `.env`로 복사해야 합니다.
