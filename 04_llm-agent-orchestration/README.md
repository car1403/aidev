...
이 과정을 마치면 다음 내용을 설명하고 구현할 수 있어야 합니다.
...
문서 구조는 직접 수업 중 헷갈리지 않도록 단순하게 유지합니다.
...
수업에서는 단원별 `.venv`를 기본 흐름으로 사용하고, 빠른 통합 실습이나 복습용으로만 최상위 `requirements.txt`를 사용하는 것을 권장합니다.
...
이 과정에서 LangChain은 “반드시 모든 코드를 LangChain으로 작성하기 위한 필수 도구”가 아닙니다. 먼저 Python 코드로 LLM 호출, Tool 함수, 데이터 흐름을 이해한 뒤, 같은 흐름을 프롬프트 템플릿, 출력 파서, Runnable, 문서 분할 같은 구조로 정리할 때 LangChain을 사용합니다.

따라서 LangChain 단원은 프레임워크 암기가 아니라 “복잡해지는 Agent 코드를 어떤 기준으로 나누고 연결할 것인가”를 이해하는 단원으로 보면 됩니다.
...
이 과정을 마친 뒤 다음을 말로 설명하고 코드로 구현할 수 있어야 합니다.
