...
04 과정부터는 Supabase가 아니라 Docker 기반 로컬 실습을 사용합니다. 이 단원에서는 Docker Desktop에서 `pgvector`가 포함된 PostgreSQL 컨테이너를 실행하고, 그 안에 벡터 데이터를 저장합니다. Docker Compose와 AWS 배포는 `06_multi-agent-service-ops`에서 본격적으로 다룹니다.
