...
`04_llm-agent-orchestration`은 개념을 배우고 작은 예제로 확인하는 과정입니다. Prompt, Tool, RAG, Memory, LangGraph를 각각 분리해서 이해하는 데 초점을 둡니다.

`05_llm-agent-mini-project`는 04에서 배운 내용을 하나의 프로젝트에 적용하는 과정입니다. 새로운 개념을 많이 추가하기보다, 아래 질문에 답할 수 있는 실제 Agent 앱을 완성하는 데 초점을 둡니다.

```text
Agent가 어떤 문제를 해결하는가?
State에는 어떤 정보를 저장하는가?
어떤 Tool이 필요하고 언제 호출되는가?
오류가 생기면 어떻게 감지하고 다시 시도하는가?
최종 결과를 화면과 보고서로 어떻게 보여주는가?
```

따라서 05에서는 `개념 설명 -> 예제 실행`보다 `프로젝트 요구사항 -> 설계 문서 -> 구현 -> 테스트 -> 발표` 흐름을 중심으로 진행합니다.
