...
이 문서는 `05_llm-agent-mini-project`가 이전 과정인 `04_llm-agent-orchestration`과 어떻게 연결되는지 정리합니다.

05는 새로운 이론을 처음부터 다시 배우는 과정이 아닙니다. 04에서 배운 Agent 구성 요소를 하나의 미니 프로젝트로 묶어 보는 단계입니다.
...
| 중심 활동 | Prompt, Tool, RAG, Memory, LangGraph 예제 실행 | 일정 조정 Agent 설계와 구현 |
| 데이터 | 작은 예제와 단원 실습 데이터 | Mock 일정 데이터와 선택 확장 데이터 |
