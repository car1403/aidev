...
Docker Compose, AWS 배포, GitHub Actions, 서비스 운영 자동화는 `06_multi-agent-service-ops`에서 다룹니다.
...
| 실행 가능한 프로젝트 | CLI 또는 Streamlit에서 실행 가능한 Agent 코드 |
| 최종 발표 자료 | 문제, 구조, 실행 결과, 개선점을 설명하는 발표 문서 |
...
4. `04_test-checklist.md` 기준으로 정상/실패 요청을 테스트합니다.
5. `05_final-presentation.md` 기준으로 발표 자료를 정리합니다.
