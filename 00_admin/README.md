...
이 폴더는 강사만 보는 관리 폴더가 아니라, **강사와 학생이 같이 열어 보면서 수업을 시작할 수 있는 운영 안내서**입니다. 학생은 여기에서 개발 환경 준비 방법, VS Code 사용법, Markdown 문서 보는 법, `.venv` 사용법, Codex 설치와 로그인 방법, OpenAI 계정과 결제의 차이, 과정별 학습 순서, 프로젝트 제출 기준을 확인합니다.

강사는 여기에서 수업 진행 순서, 체크리스트, 평가표, 프로젝트 운영 자료를 확인합니다.
...
학생들과 같이 볼 때는 번호가 붙은 새 폴더를 기준으로 안내하면 됩니다.
...
| `03_student-guides` | 학생이 직접 따라 하는 설치, VS Code, PowerShell, `.venv`, Markdown, Codex, OpenAI 계정 안내 |
...
ChatGPT에서 Codex를 사용하는 것과, Python 코드에서 OpenAI API를 호출해 비용이 발생하는 것은 별개의 흐름입니다. 수업에서 API 키를 사용하는 실습은 강사가 필요한 시점에 별도로 안내합니다.
...
03 미니 프로젝트에서는 배포를 필수로 보지 않고, 수업 시간과 학생 수준에 따라 선택 실습으로 진행합니다.
